...
def _build_upstream_url(api_client, namespace: str, vm: str) -> str:
    """Build the KubeVirt VNC subresource WebSocket URL."""
...
async def _relay(source, dest, label: str):
    """Relay messages from source to dest."""
    try:
        while True:
...
async def vnc_proxy(websocket: WebSocket, cluster: str, namespace: str, vm: str):
    ...
    try:
        ...
        logger.debug("VNC client disconnected for %s/%s", namespace, vm)
    except Exception as exc:
        logger.warning("VNC proxy error for %s/%s: %s", namespace, vm, exc)
