...
- **Backend only** — run `make backend-test`. Tests use `unittest.mock` to mock K8s API calls and FastAPI `TestClient`.
- **No frontend tests** currently exist.
- Always run `make backend-test` after modifying backend code.
- Always run `npm run build` in `frontend/` after modifying frontend code (catches TypeScript errors).
